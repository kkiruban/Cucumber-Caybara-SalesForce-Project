...
Then(/^I able to see "([^"]*)" "([^"]*)" details$/) do |name, business_hours|
  begin
    sleep 4
    arg = getDetails 'GeneralInformation'

    if page.has_css?(".businessHours")
      puts "Successfully see the the #{business_hours} section"
      ...
      within(".businessHours") do
        if first("tbody").all("tr").count > 0
          ...
          if first("tbody").all("tr")[0].all("td")[0].text == arg["NALASunday"] && (first("tbody").all("tr")[0].all("td")[1].text == arg["BusinessHoursSunday"])
            puts "Successfully see the the #{arg["NALASunday"]}: #{arg["BusinessHoursSunday"]}"
          else
            putstr "Failed to see the #{arg["NALASunday"]}: #{arg["BusinessHoursSunday"]}"
          end

          if first("tbody").all("tr")[1].all("td")[0].text == arg["NALAMonday"] || ($monday_hours == arg["BusinessHoursMonday"])
            puts "Successfully see the the #{arg["NALAMonday"]}: #{arg["BusinessHoursMonday"]}"
          else
            putstr "Failed to see the #{arg["NALAMonday"]}: #{arg["BusinessHoursMonday"]}"
          end


          if first("tbody").all("tr")[2].all("td")[0].text == arg["NALATuesday"] && first("tbody").all("tr")[2].all("td")[1].text == arg["BusinessHoursTuesday"]
            puts "Successfully see the the #{arg["NALATuesday"]}: #{arg["BusinessHoursTuesday"]}"
          else
            putstr "Failed to see the #{arg["NALATuesday"]}: #{arg["BusinessHoursTuesday"]}"
          end


          if first("tbody").all("tr")[3].all("td")[0].text == arg["NALAWednesday"] && first("tbody").all("tr")[3].all("td")[1].text == arg["BusinessHoursWednesday"]
            puts "Successfully see the the #{arg["NALAWednesday"]}: #{arg["BusinessHoursWednesday"]}"
          else
            putstr "Failed to see the #{arg["NALAWednesday"]}: #{arg["BusinessHoursWednesday"]}"
          end


          if first("tbody").all("tr")[4].all("td")[0].text == arg["NALAThursday"] && first("tbody").all("tr")[4].all("td")[1].text == arg["BusinessHoursThursday"]
            puts "Successfully see the the #{arg["NALAThursday"]}: #{arg["BusinessHoursThursday"]}"
          else
            putstr "Failed to see the #{arg["NALAThursday"]}: #{arg["BusinessHoursThursday"]}"
          end


          if first("tbody").all("tr")[5].all("td")[0].text == arg["NALAFriday"] || ($friday_hours == arg["BusinessHoursFriday"])
            puts "Successfully see the the #{arg["NALAFriday"]}: #{arg["BusinessHoursFriday"]}"
          else
            putstr "Failed to see the #{arg["NALAFriday"]}: #{arg["BusinessHoursFriday"]}"
          end


          if first("tbody").all("tr")[6].all("td")[0].text == arg["NALASaturday"] && first("tbody").all("tr")[6].all("td")[1].text == arg["BusinessHoursSaturday"]
            puts "Successfully see the the #{arg["NALASaturday"]}: #{arg["BusinessHoursSaturday"]}"
          else
            putstr "Failed to see the #{arg["NALASaturday"]}: #{arg["BusinessHoursSaturday"]}"
          end
      ...
      putstr "Failed to see the #{business_hours} section"
    end
  rescue Exception => ex
...
And(/^I verify users profile$/) do
  begin
    ...
    if page.has_css?(".fewerMore")
      ...
      putstr "Failed to see the Fewer More pagination tab"
    end
...
And(/^I "([^"]*)" should match "([^"]*)" tab in config guid$/) do |user_profile, users|
  begin
    ...
    workbook = excel.Workbooks.Open(arg["File_Path"])
