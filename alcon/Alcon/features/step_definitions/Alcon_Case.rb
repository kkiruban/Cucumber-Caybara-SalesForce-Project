...
#      else
#        puts "Case Milestone is not equal to 720 mins"
#      end
...
#      else
#        puts "Case Milestone is not equal to 1440 mins"
#      end
#      @@target_date = table.all('tr')[1].all('td')[2].text.to_s
#      puts "Target Date is #{@@target_date}"
#    end
...
Then(/^I check case milestone and target date value for reporting request$/) do
  sleep 5
  within all('.pbBody')[4] do
    table=all("tbody")[0]
    sleep 5
...
Then(/^I check case milestone and target date value for data update request$/) do
  sleep 5
  within all('.pbBody')[5] do
    table=all("tbody")[0]
    sleep 5
...
Then(/^I check the picklist value of status field$/) do
 begin
    sleep 5
    arg = getCaseDetails 'case_details'
    if page.has_select?("cas7")
      puts "Status picklist field is present and a picklist field"
      within all('.pbSubsection')[0] do
        actualvalue =find(:xpath, "//*[contains(@name, 'cas7')]").all('option').collect(&:text)
...
Then(/^I enter the mandatory field values under new case for reporting request$/) do
  ...
  Given(/^I should login as a "([^"]*)"$/) do |user_role|
    ...
    if user_role == "OpsManager"
      fill_in "Username",:with => "google.opsmgr@servicesource.com.sitgoog1p"
      fill_in "Password",:with => "testgoogle123!"
      find(:id,"Login").click
      puts "Logged in successfully"
      sleep 3
    elsif user_role == "OpsRep"
      fill_in "Username",:with => "google.opsrep@servicesource.com.sitgoog1p"
      fill_in "Password",:with => "testgoogle123!"
      find(:id,"Login").click
      puts "Logged in successfully"
      sleep 3
      ...
      fill_in "Username",:with => "google.salesrep@servicesource.com.sitgoog1p"
      fill_in "Password",:with => "testgoogle123!"
      find(:id,"Login").click
      puts "Logged in successfully"
      sleep 3
    elsif user_role == "SalesOps"
      fill_in "Username",:with => "google.salesops@servicesource.com.sitgoog1p"
      fill_in "Password",:with => "testgoogle123!"
      find(:id,"Login").click
      puts "Logged in successfully"
      sleep 3
...
When(/^I check the request escalation check box$/) do
  if page.has_field?("00N3600000JAKl1")
    find(:id, "00N3600000JAKl1").click
...
When(/^I check priority and escalated value$/) do
  sleep 2
  within all('.pbSubsection')[0] do
    if page.has_xpath?("//td[contains(text(),'Priority')]/following-sibling::td/div[contains(text(), 'Low')]")
...
Then(/^I get the just created case number$/) do
  sleep 2
  $case_number = find(:xpath,".//*[@id='bodyCell']/div[1]/div[1]/div[1]/h2").text
  puts "Case Number: #{$case_number}"
end

When(/^I click on just created "([^"]*)" case number$/) do |arg1|
sleep 5
  ...
  within all(".pbBody")[0] do
    sleep 3
    click_on($case_number, :match => :first)
    sleep 2
  end
end

When(/^I check that approval history section is visible$/) do
 sleep 4
 if page.has_xpath?("//tr/td/h3[text()='Approval History']")
   puts "Approval History section is visible"
 else
   writeFailure "Approval History section is not visible"
 end
end
...
When(/^I update "([^"]*)" picklist value to "([^"]*)"$/) do |arg1, arg2|
  if page.has_field?("Sub Case Type")
    select(arg2, :from => arg1)
  end
end

Then(/^I click on save button which should show error message$/) do
  sleep 2
  ...
  sleep 5
  if page.has_content?("The following field cannot be blank : Renewal Status when Sub Case Type = Closed Sale, House Account or No Service")
    puts "shows error message: The following field cannot be blank : Renewal Status when Sub Case Type = Closed Sale, House Account or No Service"
  else
...
Then(/^I check the escalated check box$/) do
 if page.has_field?("IsEscalated")
    find(:id, "IsEscalated").click
    puts "Escalated field is checked"
  else
    writeFailure "Escalated field is not available"
  end
end
