...
After do |scenario|
  if scenario.failed?
    filename = "./screenshots/failed_scenario _screenshots/#{scenario.name}_#{Time.now.strftime('%Y-%m-%d-%H-%M-%S')}.png"
    File.open(filename,'wb') do |f|
      f.write(Base64.decode64(page.driver.browser.screenshot_as(:base64)))
      embed(filename, 'image/png;base64')
    end
  else
    filename = "./screenshots/sucess_scenario_sceenshots/#{scenario.name}_#{Time.now.strftime('%Y-%m-%d-%H-%M-%S')}.png"
    File.open(filename,'wb') do |f|
      f.write(Base64.decode64(page.driver.browser.screenshot_as(:base64)))
      embed(filename, 'image/png;base64')
    end
