...
set _filename=Result\GE_Opps_Rep_%_my_datetime%.html
rem set _errfilename=temp\result%_my_datetime%.html

rem navigage to the directory where the batch file and scripts exists
cd /d %~dp0
cucumber features --tags @opp-all-users,@quotes-all-users,@asset-all-users,@account-all-users,@case-all-users,@contact-all-users --format html --out %_filename%  UserRole="opsrepsit"
