...
Then(/^I verify that "([^"]*)" is available and an active user on click of "([^"]*)"$/) do |arg1, arg2|
  within all('.bNext')[0] do
    ...
    if page.has_xpath?('//span[text()="'"#{arg2}"'" and (@class="listItemSelected" or @class="listItemPad")]')
      find(:xpath, '(//span[text()="'"#{arg2}"'" and (@class="listItemSelected" or @class="listItemPad")])[1]').click
      puts "The link #{arg2} has been clicked"
      sleep 8
    else
      writeFailure "FAIL to click the link #{arg2}"
    end
      within('.pbBody') do
