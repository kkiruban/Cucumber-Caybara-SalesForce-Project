...
Then(/^I click on renewal opportunity link$/) do
  sleep 3
  page.find(:xpath, "//*[@id='CF00N4100000PEZCQ_ileinner']/a").click
  sleep 5
end
...
Then(/^I should see Booking Type picklist values$/) do
  actualvalues =find_field("Booking Type", match: :prefer_exact).all('option').collect(&:text)
  puts "actual values = #{actualvalues}"
  expectedvalues = ["--None--","New Contract Existing Asset","New Contract New Asset","Renewal"]
  puts "expectedvalues =  #{expectedvalues}"
  if actualvalues.sort == expectedvalues.sort
    puts "expected values are visible in Booking Type picklist"
  else
    writeFailure "expected values are not visible in Booking Type picklist"
  end
end
...
Then(/^I should see "([^"]*)" field with read and write permission for all users$/) do |field_name|
...
Then(/^I should see "([^"]*)" field with read and write permission for OpsManager and OpsRep user$/) do |field_name|
...
Then(/^I should see Opportunity Type picklist values$/) do
  if page.has_field?('Opportunity Type')
    actualvalues =find_field("Opportunity Type", match: :prefer_exact).all('option').collect(&:text)
    puts "actual values = #{actualvalues}"
    expectedvalues = ["Existing Customer - Service Renewal"]
    puts "expectedvalues =  #{expectedvalues}"
    if actualvalues.sort == expectedvalues.sort
...
When(/^I select "([^"]*)" from "([^"]*)" in opportunity product edit page$/) do |value, field_name|
...
Then(/^I should see Renewal Status picklist values$/) do
  if page.has_field?('Renewal Status')
    ...
    puts "actual values = #{actualvalues}"
    expectedvalues =  ["--None--", "CS - Backdated - BKD", "CS - Co-term Long - CTL", "CS - Co-term Short - CTS", "CS - Discount - DIS", "CS - Multi-year Advance - MYA", "CS - Pricing Change - PRC", "CS - Re-Cert Fee Included - RCT", "CS - Renewed at Par - R@P", "CS - Service Downgrade - DNG", "CS - Service Upgrade - UPG", "CS - Uncovered - UNC", "HA - Bad Data - BDT", "HA - Cancelled - CNL", "HA - Covered - COV", "HA - Duplicate - DUP", "HA - End Of Support - EOL", "HA - Evergreen Billing - EVG", "HA - International - INT", "HA - Lease - LEA", "HA - OEM Customer - OEM", "HA - Other - OTH", "HA - Product Return - PRT", "HA - Sales Pull Back - SPB", "NS - Client Product Replacement - PTR", "NS - Competitive DVAR - SCV", "NS - Competitive Product Replacement - PCP", "NS - Competitive Service Loss Other - SCS", "NS - Customer Cost-Benefit Decision - SCB", "NS - Customer No Longer Exists - RNE", "NS - Customer Satisfaction Driven - SSD", "NS - End of Service Life - PEL", "NS - Other Data Management - RDM", "NS - Product Decommissioned - PPD", "NS - Third Party Maintenance - SPM", "NS - Unresponsive End User - REU", "NS - Unresponsive VAR - RUV", "NS - VAR No Service - RVN", "Open"]
    puts "expectedvalues =  #{expectedvalues}"
    if actualvalues.sort == expectedvalues.sort
...
Then(/^I should see Partner Account (\d+) Type picklist values$/) do |value|
  if value == "1"
    if page.has_field?("Partner Account 1 Type")
      actualvalues =find_field('Partner Account 1 Type').all('option').collect(&:text)
      puts "actual values = #{actualvalues}"
      expectedvalues = ["--None--","Reseller","Distributor"]
      puts "expectedvalues =  #{expectedvalues}"
      if actualvalues.sort == expectedvalues.sort
        puts "expected values are visible in Partner Account 1 Type picklist"
      else
        writeFailure "expected values are not visible in Partner Account 1 Type picklist"
      end
    else
      writeFailure "Partner Account 1 Type field is not visible"
    end
  else
    if page.has_field?("Partner Account 2 Type")
      actualvalues =find_field('Partner Account 2 Type').all('option').collect(&:text)
      puts "actual values = #{actualvalues}"
      expectedvalues = ["--None--","Reseller","Distributor"]
      puts "expectedvalues =  #{expectedvalues}"
      if actualvalues.sort == expectedvalues.sort
        puts "expected values are visible in Partner Account 2 Type picklist"
      else
        writeFailure "expected values are not visible in Partner Account 2 Type picklist"
      end
    else
      writeFailure "Partner Account 2 Type field is not visible"
    end
  end
end
...
Then(/^I should see Client Region picklist values$/) do
  ...
  puts "actual values = #{actualvalues}"
  expectedvalues = ["--None--", "Education-K12", "Named Hospitality", "Named Retail/Transportation and Logistics", "Strategic Accounts and Segments", "US Territories", "LATAM", "ANZ", "ASEAN", "GCR", "India/SA", "Japan", "Korea", "DACH", "Eastern/Nordics", "Middle East", "Partner Led EMEA", "Southern", "UKBen"]
  puts "expectedvalues =  #{expectedvalues}"
  if actualvalues.sort == expectedvalues.sort
...
Then(/^I should see Client sales stage picklist values$/) do
  actualvalues =find_field("Client Sales Stage", match: :prefer_exact).all('option').collect(&:text)
  puts "actual values = #{actualvalues}"
  expectedvalues = ["--None--", "Pre Stage: Sales Qualified Opportunity", "Pre Stage: Received/Prospecting", "1. Buyer Stimulated", "2. Buyer Defining Problem", "3. Buyer Evaluating Options", "4. Buyer Recommending a Solution", "5. Buyer Making a Final Decision", "6. Buyer Authorizes and Implements", "Closed Lost", "Duplicate Opportunity"]
  puts "expectedvalues =  #{expectedvalues}"
  if actualvalues.sort == expectedvalues.sort
  ...
  select('Pre Stage: Received/Prospecting', :from => 'Client Sales Stage', match: :prefer_exact)
end

Then(/^I should see Forecast picklist values$/) do
  actualvalues =find_field("Forecast Category", match: :prefer_exact).all('option').collect(&:text)
  puts "actual values = #{actualvalues}"
  expectedvalues =  ["--None--", "Non-Forecastable"]
  puts "expectedvalues =  #{expectedvalues}"
  if actualvalues == expectedvalues
...
Then(/^I should see Direct\/Channel picklist values$/) do
  actualvalues =find_field("Direct/Channel", match: :prefer_exact).all('option').collect(&:text)
  puts "actual values = #{actualvalues}"
  expectedvalues = ["--None--","Direct","Direct VAR","Distributor"]
  puts "expectedvalues =  #{expectedvalues}"
  if actualvalues.sort == expectedvalues.sort
...
Then(/^I enter data to all mandatory fields under renewable line item$/) do
  ...
  within('.pbBody') do
    ...
    if (ENV['UserRole'] == "OperationManagerSit")
      click_on "Operations Manager Extreme Networks"
      puts "assigned to Operations Manager APAC Extreme Networks"
      sleep 2
    else if (ENV['UserRole'] == "OperationRepSit")
        click_on "Operations Rep Extreme Networks"
        puts "assigned to Operations Rep NALA Extreme"
        sleep 2
      else if (ENV['UserRole'] == "SalesManagerSit")
          click_on "Sales Manager Extreme Networks"
          puts "assigned to Sales Manager APAC Extreme Networks"
          sleep 2
        else if (ENV['UserRole'] == "SalesRepSit")
            click_on "Sales rep Extreme Networks"
            puts "assigned to Sales rep APAC Extreme Networks"
            sleep 2
              ...
              sleep 2
                ...
                click_on "Sales Ops NALA Extreme"
                puts "assigned to Sales Ops NALA Extreme"
                sleep 2
              end
            end
          end
...
Then(/^I should see Client Territory picklist values$/) do
  ...
  puts "actual values = #{actualvalues}"
  expectedvalues = ["--None--","Americas","APAC","EMEA"]
  puts "expectedvalues =  #{expectedvalues}"
  if actualvalues == expectedvalues
